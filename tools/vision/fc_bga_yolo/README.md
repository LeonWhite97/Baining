...
The converter rejects path traversal, missing/extra lights, duplicate sample IDs, invalid YOLO rows, decode failures, and unequal dimensions. The validator also rejects image/label mismatches, non-finite or out-of-range boxes, hash changes, and `group_id` leakage across train/val/test.
...
Conflicting labels stop apply with `LABEL_CONFLICT`; boxes are never merged automatically. Canonical priority is test, val, then train.
...
python tools/vision/fc_bga_yolo/download_models.py --model yolov8n.pt
python tools/vision/fc_bga_yolo/train.py --config tools/vision/fc_bga_yolo/configs/train_smoke.yaml
```

The dataset's `OK/NG` labels are separate from the formal seven-class contract.
...
```powershell
python tools/vision/fc_bga_yolo/download_models.py --model yolov8s.pt
python tools/vision/fc_bga_yolo/train.py --config tools/vision/fc_bga_yolo/configs/train_poc.yaml --check-only
python tools/vision/fc_bga_yolo/train.py --config tools/vision/fc_bga_yolo/configs/train_poc.yaml
```

The PoC profile uses 1280 pixels, 100 epochs, patience 20, batch 8, seed 42, and deterministic mode. It validates formal data before importing Ultralytics, evaluates the independent test split, and writes `model_metadata.json` beside `best.pt`.
...
python tools/vision/fc_bga_yolo/export_model.py --model D:/aoi-runs/weights/best.pt --metadata D:/aoi-runs/weights/model_metadata.json --format onnx --device cpu
```
...
The adapter checks class order, input contract, metadata, and the model SHA-256 before inference. Missing dependencies, changed files, load failures, invalid output, or runtime errors persist an auditable `REVIEW` with reason `MODEL_UNAVAILABLE`. With this first detector, no-box and low-score results also remain REVIEW because `normal_confidence` is intentionally zero.
